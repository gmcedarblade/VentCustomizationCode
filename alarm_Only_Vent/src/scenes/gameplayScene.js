var GamePlayScene = function(game, stage, args)
{
  ...
  */
  ...
  var evaluate_patient = function()
  {
    ...
    if(in_volume < 0.499 || in_volume > 0.671) { console.log("vol");  return false; }
    if(in_rate < 9.9 || in_rate > 16.1) { console.log("rate");  return false; }
    if(in_peep < 4.9 || in_peep > 7.1) { console.log("peep"); return false; }
    if(in_oxy < 49.9 || in_oxy > 100.1) { console.log("oxy");  return false; }
...
//      update_alarms();
    ...
    dismiss_submit_btn = new ButtonBox(bogus_canv.width/2+75,580,100,40, function()
    {
      cur_screen = SCREEN_VENTILATOR;
      if(evaluate_patient() && evaluate_alarms()) playerSuccess();
      else playerFailure();
    });
    dismiss_submit_btn.title = "Continue";
