var GamePlayScene = function(game, stage, args)
{
  ...
  */
  ...
  var evaluate_patient = function()
  {
    ...
    if(fdisp(in_volume, 3) < 0.280 || fdisp(in_volume, 3) > 0.390) { console.log("vol");  return false; }
  ...
  {
    if(!blip_running)
    // Original code for reference                                                                                                                              { console.log("blip"); return false; }
    ...
    console.log("max emv: " + fdisp(in_alarm_max_exhale_minute_volume,0) + "\n");
    console.log("min emv: " + fdisp(in_alarm_min_exhale_minute_volume,0) + "\n");
    ...
    if(fdisp(in_alarm_max_exhale_minute_volume,0) > 12.1 || fdisp(in_alarm_max_exhale_minute_volume,0) < 7.9) { console.log("emv alarm max");      return false; }
    if(fdisp(in_alarm_min_exhale_minute_volume,0) >  4.1 || fdisp(in_alarm_min_exhale_minute_volume,0) <  1.9) { console.log("emv alarm min");      return false; }
  ...
  var alarm = function(min_select,max_select,min_selected,max_selected,min_text,max_text,text,minmin_text,maxmax_text,title,label)
  {
    ...
    self.draw = function()
    {
      ...
      ctx.fillStyle = green;
      if(max_selected()){
        canv.fillRoundRect(self.max_box.x,self.max_box.y,self.max_box.w,self.max_box.h,5);
        ctx.fillStyle = white;
        ctx.font = "14px Helvetica";
  ...
  var exitNotificationScreenSettings = function() {		
    ...
    yd = 35;		
    ctx.font = "35px Helvetica";		
    ctx.fillText("Waiting to Verify",x+15,y); y+=yd;		
    //ctx.fillText("if you are",x+75,y); 
    y+=yd;		
    ctx.fillText("Alarms...",x+75,y); y+=yd;		
    y = 380;		
    yd = 25;		
  ...
  self.ready = function()
  {
    ...
    {
      /*
      Commented out so that there is no ablity for the
      user to commit changes the vent settings.
      */
//      blip_t = 0;
...
//      update_alarms();
    ...
    dismiss_submit_btn = new ButtonBox(bogus_canv.width/2+25,580,145,40, function()
    {
      cur_screen = SCREEN_VENTILATOR;
      if(evaluate_patient() && evaluate_alarms()) playerSuccess();
      else playerFailure();
    });
    dismiss_submit_btn.title = "Verify Alarms";
  ...
  self.draw = function()
  {
    ...
    else if(cur_screen == SCREEN_VENTILATOR)
    {
      ...
      canv.fillStyle = green;
      ctx.font = "18px Helvetica";
      ctx.fillText("Verify Alarm Settings",next_btn.x+8,next_btn.y+next_btn.h/2+7);
