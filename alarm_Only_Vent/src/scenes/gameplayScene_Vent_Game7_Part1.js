var GamePlayScene = function(game, stage, args)
{
  ...
  */
  ...
  var evaluate_alarms = function()
  {
    ...
    if(fdisp(in_alarm_max_exhale_minute_volume,1) > 15.1 || fdisp(in_alarm_max_exhale_minute_volume,1) < 10.9) { console.log("emv alarm max");      return false; }
    if(fdisp(in_alarm_min_exhale_minute_volume,1) >  4.1 || fdisp(in_alarm_min_exhale_minute_volume,1) <  1.9) { console.log("emv alarm min");      return false; }
  ...
  var exitNotificationScreenSettings = function() {		
    ...
    yd = 35;		
    ctx.font = "35px Helvetica";		
    ctx.fillText("Waiting to Verify",x+25,y-35); y+=yd;		
    //ctx.fillText("if you are",x+75,y); 
    y+=yd;		
    ctx.fillText("Alarms...",x+80,y-25); y+=yd;		
    y = 380;		
    yd = 25;		
...
//      update_alarms();
    ...
    dismiss_submit_btn = new ButtonBox(bogus_canv.width/2+35,580,145,40, function()
    {
      cur_screen = SCREEN_VENTILATOR;
