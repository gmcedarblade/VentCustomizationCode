var GamePlayScene = function(game, stage, args)
{
  ...
  default values
  */
  var in_volume   = localStorage.getItem("ventPlayerSet") ? object.volume:.450;
  
  var in_pressure = localStorage.getItem("ventPlayerSet") ? object.pressure:lerp(min_in_pressure, max_in_pressure, norm_in_pressure);
  
  var in_rate     = localStorage.getItem("ventPlayerSet") ? object.rate:18;
  var in_flow     = localStorage.getItem("ventPlayerSet") ? object.flow:30;
  var in_oxy      = localStorage.getItem("ventPlayerSet") ? object.oxygen:100;
  var in_itime    = localStorage.getItem("ventPlayerSet") ? object.itime:lerp(min_in_itime  ,  max_in_itime  ,  norm_in_itime);
  var in_peep     = localStorage.getItem("ventPlayerSet") ? object.peep:8;
  
  // Start of setting min and max display values for alarms
  
  var norm_in_alarm_min_pressure             = mapVal(min_in_alarm_pressure,             max_in_alarm_pressure,             0,1, localStorage.getItem("alarmPlayerSet") ? alarmParse.minPressureAlarm:18);
  var norm_in_alarm_max_pressure             = mapVal(min_in_alarm_pressure,             max_in_alarm_pressure,             0,1, localStorage.getItem("alarmPlayerSet") ? alarmParse.maxPressureAlarm:28);
  var norm_in_alarm_min_rate                 = mapVal(min_in_alarm_rate,                 max_in_alarm_rate,                 0,1, localStorage.getItem("alarmPlayerSet") ? alarmParse.minRateAlarm:13);
  var norm_in_alarm_max_rate                 = mapVal(min_in_alarm_rate,                 max_in_alarm_rate,                 0,1, localStorage.getItem("alarmPlayerSet") ? alarmParse.maxRateAlarm:23);
  var norm_in_alarm_min_exhale_minute_volume = mapVal(min_in_alarm_exhale_minute_volume, max_in_alarm_exhale_minute_volume, 0,1, localStorage.getItem("alarmPlayerSet") ? alarmParse.minFlowAlarm:3.1);
  var norm_in_alarm_max_exhale_minute_volume = mapVal(min_in_alarm_exhale_minute_volume, max_in_alarm_exhale_minute_volume, 0,1, localStorage.getItem("alarmPlayerSet") ? alarmParse.maxFlowAlarm:13.1);
  ...
  var norm_in_alarm_max_apnea = mapVal(min_in_alarm_apnea , max_in_alarm_apnea , 0,1, localStorage.getItem("alarmPlayerSet") ? alarmParse.maxApneaAlarm:20);
  ...
  var evaluate_patient = function()
  {
    ...
    if(fdisp(in_volume, 3) < 0.290 || fdisp(in_volume, 3) > 0.310) { console.log("vol");  return false; }
    if(fdisp(in_rate, 1) < 25.5 || fdisp(in_rate,1) > 26.5) { console.log("rate");  return false; }
    if(fdisp(in_peep, 1) < 11.5 || fdisp(in_peep,1) > 12.5) { console.log("peep"); return false; }
    if(fdisp(in_oxy, 2) < 95 || fdisp(in_oxy, 2) > 105) { console.log("oxy");  return false; }
  ...
  var evaluate_alarms = function()
  {
    ...
    if(fdisp(in_alarm_max_pressure,0)             > 34.1 || fdisp(in_alarm_max_pressure,0)             < 24.9) { console.log("pressure alarm max"); return false; }
    if(fdisp(in_alarm_min_pressure,0)             > 15.1 || fdisp(in_alarm_min_pressure,0)             < 6.9) { console.log("pressure alarm min"); return false; }
    
    if(fdisp(in_alarm_max_rate,0)                 > 35.1 || fdisp(in_alarm_max_rate,0)                 < 29.9) { console.log("rate alarm max");     return false; }
    if(fdisp(in_alarm_min_rate,0)                 > 21.1 || fdisp(in_alarm_min_rate,0)                 <  15.9) { console.log("rate alarm min");     return false; }
    
    if(fdisp(in_alarm_max_exhale_minute_volume,1) > 11.1 || fdisp(in_alarm_max_exhale_minute_volume,1) < 7.9) { console.log("emv alarm max");      return false; }
    if(fdisp(in_alarm_min_exhale_minute_volume,1) >  7.1 || fdisp(in_alarm_min_exhale_minute_volume,1) <  3.9) { console.log("emv alarm min");      return false; }
  ...
  var update_alarms = function()
  {
    ...
    getAlarmSettings(in_alarm_max_pressure, in_alarm_min_pressure, in_alarm_max_rate, in_alarm_min_rate, in_alarm_max_exhale_minute_volume, in_alarm_min_exhale_minute_volume, in_alarm_max_apnea, in_alarm_min_apnea);
  }
  ...
  self.ready = function()
  {
    ...
    {
      /*
      Comment out so there is no ability for the user to 
      make changes and be evaluated if needed
      */
     cur_screen = SCREEN_VENTILATOR;
     if(evaluate_patient() && evaluate_alarms()) playerSuccess();
     else playerFailure();
    });
    dismiss_submit_btn.title = "Apply Changes";
  ...
  self.tick = function()
  {
    ...
    if(blip_t == blip_rate || floor(((blip_t-patient_volume_graph.data.itime)-blip_rate)/fulllen) != floor((blip_t-patient_volume_graph.data.itime)/fulllen))
    {
      ...
      norm_patient_mean_pressure = commit_in_volume/expected_volume * 0.18; //0.15 is ~ good readout
  ...
  self.draw = function()
  {
    ...
    else if(cur_screen == SCREEN_VENTILATOR)
    {
      ...
      ctx.fillText("Apply Changes",next_btn.x+8,next_btn.y+next_btn.h/2+7);
